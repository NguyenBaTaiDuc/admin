...
import { AndroidOutlined, InfoCircleOutlined } from "@ant-design/icons";
...
const Sidebar: React.FC<SideBarProps> = ({ onToggleCollapse }) => {
  ...
  const mobileSidebar = () => {
    setIsMobileTabberOpen(!isMobileTabbarOpen)
  }
  const toggleSidebar = () => {
  ...
  return (
    ...
    <>
      ...
      {isMobileTabbarOpen && (
        <div
          className="fixed inset-0 bg-black/15 bg-opacity-30 backdrop-blur-sm z-40 sm:hidden"
      ...
      <div
        className={` W-72 flex flex-col justify-between fixed top-0 left-0 h-screen p-4 bg-custom-sidebar transition-transform  duration-400 ease-in-out
           ${isMobileTabbarOpen ? "translate-x-0" : "-translate-x-full"} sm:translate-x-0
           ${isCollapsed ? "w-17 rounded-md" : "w-72"}
      ...
      >
        ...
        <div className="flex flex-col flex-end">
          ...
          {/* Menu */}
          <nav className={"flex flex-col w-full items-start px-2 space-y-5 "}>
            <NavLink
            ...
            >
              ...
              />
              {!isCollapsed && <p className="text-white text-base">{t('home')}</p>}
            </NavLink>
            ...
            >
              ...
              <AndroidOutlined
                ...
                className={`text-white ${isCollapsed ? "text-2xl" : "text-xl"}`}
              />
              {!isCollapsed && <p className="text-white text-base">{t('ai_content')}</p>}
            </NavLink>
            ...
            >
              ...
              description={"Oganize and manage affiliate links to increase revenue through partnerships and collaborations"}/>
                  ...
                  {/* Text */}
                  {!isCollapsed && (
                    <p className="text-white text-base">{t('Social Media')}</p>
                  )}
                </div>
              ...
              {isSocialOpen && !isCollapsed && (
                <div className="flex flex-col mt-4 space-y-1 w-full px-2">
                  <NavLink
                    to="/Media_Management"
                    className=" pl-12 w-full block text-white text-sm hover-gbsidebar rounded-md py-2 px-2"
                  >
                    {t('Media Management')}
                  </NavLink>
                  <NavLink
                    to="/post-schedule"
                    className="w-full block text-white text-sm hover-gbsidebar rounded-md py-2 px-2 pl-12"
                  >
                    {t('Post Schedule')}
                  </NavLink>
                </div>
            ...
            >
              ...
              <MergeOutlined
                ...
                className={`text-white ${isCollapsed ? "text-2xl" : "text-xl"}`}
              />
              {!isCollapsed && <p className="text-white text-base">{t('Traffic Grow Center')}</p>}
            </NavLink>
            ...
            >
              ...
              <SettingOutlined
                ...
                className={`text-white ${isCollapsed ? "text-2xl" : "text-xl"}`}
              />
              {!isCollapsed && <p className="text-white text-base">{t('Settings')}</p>}
            </NavLink>
        ...
        <div
          className={`relative w-full h-10 rounded-md cursor-pointer flex items-center gap-2 text-white text-sm mb-8 opacity-90 hover-gbsidebar
        ...
        >
          ...
          {t('Language')}
          {isCollapsed && <span className="text-base"> {t('language')}</span>}
          {
            !isCollapsed && isLangOpen && (
              <div className="absolute bottom-full mb-2 left-0 w-full bg-primary text-white rounded-md shadow-md z-50">
                <button
                  ...
                  className="flex flex-row justify-start w-full text-left px-8 py-2 hover-gbsidebar"
                >
                <img src={image} alt="Image" className="w-6 h-6 mr-3 -ml-1" />
                  English
                </button>
                <button
                  ...
                  className="flex flex-row justify-start w-full text-left px-8 py-2 hover-gbsidebar"
                >
                <img src={imageVN} alt="Image" className="w-7 h-7 -pl-1 mr-2"  />
                  Tiếng Việt
                </button>
              </div>
            )
